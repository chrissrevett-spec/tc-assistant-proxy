// api/assistant_json.js — NON-STREAM + CORS + citations + usage
module.exports = async function (req, res) {
  // --- CORS (adjust whitelist later if you want) ---
  ...
  try {
    ...
    const KEY = process.env.OPENAI_API_KEY, AID = process.env.ASSISTANT_ID;
    if (!KEY || !AID) return res.status(500).json({ ok:false, error:"Server not configured (env vars)." });
    ...
    const rR = await fetch(U(`/threads/${threadId}/runs`), {
      method:"POST", headers:H, body: JSON.stringify({ assistant_id: AID })
    });
    ...
    for (const part of (assistantMsg?.content || [])) {
      if (part.type === "text" && part.text?.annotations?.length) {
        for (const a of part.text.annotations) {
          // Normalise to a simple array of labels
    ...
    return res.status(200).json({
      ...
      text,
      citations,
      usage // prompt/completion/total tokens if available
    });
  } catch (e) {
